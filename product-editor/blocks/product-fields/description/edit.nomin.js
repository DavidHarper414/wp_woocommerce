...
function DescriptionBlockEdit({ attributes, }) {
    ...
    const { isModalEditorOpen, modalEditorBlocks, hasChanged } = (0, data_1.useSelect)((select) => {
        return {
            isModalEditorOpen: select(product_editor_ui_1.wooProductEditorUiStore).isModalEditorOpen(),
            modalEditorBlocks: select(product_editor_ui_1.wooProductEditorUiStore).getModalEditorBlocks(),
            hasChanged: select(product_editor_ui_1.wooProductEditorUiStore).getModalEditorContentHasChanged(),
        };
    }, []);
