...
function Edit({ attributes, context: { isInSelectedTab }, }) {
    ...
    const { invalidateResolution } = (0, data_2.useDispatch)(data_1.experimentalProductVariationsStore);
    ...
    const { totalCountWithoutPrice } = (0, data_2.useSelect)((select) => {
        const { getProductVariationsTotalCount } = select(data_1.experimentalProductVariationsStore);
        return {
    ...
    function onSetPrices(handleUpdateAll) {
        ...
        const productVariationsListPromise = (0, data_2.resolveSelect)(data_1.experimentalProductVariationsStore).getProductVariations({
            product_id: productId,
            order: 'asc',
            orderby: 'menu_order',
            has_price: false,
