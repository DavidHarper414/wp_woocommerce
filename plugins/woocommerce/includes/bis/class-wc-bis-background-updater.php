<?php
/**
 * WC_BIS_Background_Updater class
 *
...
class WC_BIS_Background_Updater extends WC_Background_Process {
	...
	public function dispatch() {
		...
		if ( is_wp_error( $dispatched ) ) {
			WC_BIS()->log( sprintf( 'Unable to dispatch WooCommerce Back In Stock Notifications updater: %s', $dispatched->get_error_message() ), 'error', 'wc_bis_db_updates' );
		}
	}
	...
	protected function task( $callback ) {
		...
		include_once WC_BIS_ABSPATH . 'includes/wc-bis-update-functions.php';

		if ( is_callable( $callback ) ) {

			WC_BIS()->log( sprintf( '- Running %s callback...', $callback ), 'info', 'wc_bis_db_updates' );
			...
			WC_BIS()->log( $message, 'info', 'wc_bis_db_updates' );

		} else {

			$result = false;

			WC_BIS()->log( sprintf( '- Could not find %s callback.', $callback ), 'notice', 'wc_bis_db_updates' );
		}
	...
	protected function complete() {
		WC_BIS_Install::update_complete();
		parent::complete();
	}
}
