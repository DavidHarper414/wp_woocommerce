...
//add_filter('woocommerce_composite_component_options_query_args', function ($args, $query_args, $component_data) {
//	$logger = wc_get_logger();
//	$context = ['source' => 'woocommerce_composite_component_options_query_args'];
//	if (isset($_GET['pa_mudel'])) {
//		$selected_model_name = sanitize_text_field(urldecode($_GET['pa_mudel']));
//		$term = get_term_by('name', $selected_model_name, 'pa_mudel');
//		if ($term && !is_wp_error($term)) {
//			$selected_model_id = (int) $term->term_id;
//			// Ensure tax_query exists
//			if (!isset($args['tax_query'])) {
//				$args['tax_query'] = [];
//			}
//			// Add the pa_mudel filter
//			$args['tax_query'][] = [
//				'taxonomy' => 'pa_mudel',
//				'field'    => 'term_id',
//				'terms'    => [$selected_model_id],
//				'operator' => 'IN',
//			];
//			// Force WooCommerce to display results even if only one product matches
//			$args['posts_per_page'] = -1; // Ensures all results are fetched
//			$args['nopaging'] = true; // Prevents WooCommerce pagination from excluding results
//		}
//	}
//	// Log the updated query
//	$logger->info("Final WooCommerce Query Args: " . print_r($args, true), $context);
//	return $args;
//}, 10, 3);

/**
 * Filter Composite component options by a `model` URL parameter.
 */
function filter_composite_component_options_by_model( $query_args, $component_id, $composite ) {

	// Only proceed if the "model" query parameter is present and not empty.
	if ( isset( $_GET['pa_mudel'] ) && ! empty( $_GET['pa_mudel'] ) ) {

		// Sanitize the model parameter (e.g. "iPhone 11" -> "iphone-11").
		$raw_model = sanitize_text_field( wp_unslash( $_GET['pa_mudel'] ) );
		$model_slug = sanitize_title( $raw_model ); // Generates slug-like string

		// (Optional) Check component_id if you only want to filter certain components
		// For example:
		// if ( in_array( $component_id, array( 'cases', 'screen_protectors', 'charging_cables' ), true ) ) { ... }

		// Modify the tax_query so we only get products in the matching subcategory.
		// Important: make sure your product category slug is indeed `iphone-11`, `iphone-12-pro`, etc.
		$query_args['tax_query'][] = array(
			'taxonomy' => 'product_cat',
			'field'    => 'slug',
			'terms'    => array( $model_slug ),
			'operator' => 'IN',
		);
	}
...
add_filter( 'woocommerce_composite_component_options_query_args', 'filter_composite_component_options_by_model', 10, 3 );
