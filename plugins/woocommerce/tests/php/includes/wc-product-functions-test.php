...
class WC_Product_Functions_Tests extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox Test wc_uploads_is_direct_access_allowed function checks .htaccess and uses caching
	 */
	public function test_wc_uploads_is_direct_access_allowed() {
		// Create mock upload dir info
		$upload_dir = array(
				'basedir' => '/tmp/woocommerce-test'
				);

		// Create test directory
		$test_dir = $upload_dir['basedir'] . '/woocommerce_uploads';
		if (!file_exists($test_dir)) {
			mkdir($test_dir, 0777, true);
		}
		$htaccess_file_path = $test_dir . '/.htaccess';

		$transient_key = 'wc_htaccess_direct_access_' . md5($htaccess_file_path);

		// delete the file just in case it was created by a different process.
		if ( file_exists( $htaccess_file_path ) ) {
			unlink( $htaccess_file_path );
		}

		// Mock wp_get_upload_dir()
		FunctionsMockerHack::add_function_mocks(array(
					'wp_get_upload_dir' => function() use ($upload_dir) {
					return $upload_dir;
					}
					));

		// No .htaccess file exists
		$this->assertTrue(wc_uploads_is_direct_access_allowed(), 'Should return true when no .htaccess exists');

		// .htaccess exists and denies access
		file_put_contents($htaccess_file_path, 'deny from all');
		$this->assertFalse(wc_uploads_is_direct_access_allowed(), 'Should return false when .htaccess denies access');
		delete_transient($transient_key); // clean up so next asserstion load from file

		// Test 3: .htaccess exists but allows access
		file_put_contents($htaccess_file_path, 'allow from all');
		$this->assertTrue(wc_uploads_is_direct_access_allowed(), 'Should return true when .htaccess allows access');

		// Test 4: Verify caching works
		$cached_value = get_transient($transient_key);
		$this->assertEquals('allow from all', $cached_value, 'Transient should contain .htaccess content');

		// When cache is busted, value should come from file again
		delete_transient($transient_key);
		file_put_contents($htaccess_file_path, 'deny from all');
		$this->assertFalse(wc_uploads_is_direct_access_allowed(), 'Should deny access after the cache is busted and deny from all is set in the file.');
