...
class AddToCartWithOptions extends \WP_UnitTestCase {
	...
	public function test_grouped_product_selector_cta() {
		...
		// Create a simple product sold individually.
		// Create a variable product.
		// $simple_product->set_sold_individually( true );

		// First test: make the simple product to be a child of the grouped product.
		// Test that we display a quantity selector.
		$simple_product = new \WC_Product_Simple();
		$simple_product->set_price( 10 );
		...
		echo 'GROUPED PRODUCT ID: ' . $grouped_product_id . '<br>';
		...
		$this->assertStringContainsString( 'wc-block-add-to-cart-with-options-grouped-product-selector-item-cta', $markup, 'The Grouped Product Add to Cart with Options form contains a checkbox.' );
		// $this->assertStringContainsString( '<input type="number', $markup, 'The Grouped Product Add to Cart with Options form contains a checkbox.' );

		// Second test: make the simple product sold individually.
		// Test that we display a checkbox.
		// $this->assertStringContainsString( '<input type="checkbox', $markup, 'The Grouped Product Add to Cart with Options form contains a checkbox.' );

		// Third test: make the variable product to be a child of the grouped product.
		// Test that we display a button.
	}
}
