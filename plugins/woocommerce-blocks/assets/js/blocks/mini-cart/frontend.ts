...
window.addEventListener( 'load', () => {
	...
	miniCartBlocks.forEach( ( miniCartBlock, i ) => {
		...
		// Auto-open the drawer if a localStorage flag is set from the single product page.
		// Remove the flag after opening the drawer to prevent reopening on page refresh.
		const shouldOpenDrawer =
			localStorage.getItem( 'open_mini_cart_drawer' ) === 'true';
		if ( shouldOpenDrawer ) {
			localStorage.removeItem( 'open_mini_cart_drawer' );
			openDrawer();
		}
